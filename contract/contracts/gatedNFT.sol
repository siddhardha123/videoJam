...
contract TeacherNFT is ERC721 {
    ...
    mapping (uint256 => Teacher) public tokenIdToTeacher;
    
    uint256 public totalTokens;
    
    constructor() ERC721("TeacherNFT", "TNFT") {}
    
    function addTeacher(address teacherAddress, string memory name, uint256 price, string memory photo) public {
        require(msg.sender == teacherAddress, "Only teacher can add themselves");
        teachers[teacherAddress] = Teacher(name, price, photo);
    }
   ...
   function buyNFT(address teacherAddress) public payable {
    ...
    totalTokens += 1;
    payable(teacherAddress).transfer(msg.value);
}

    
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        Teacher memory teacher = tokenIdToTeacher[tokenId];
        string memory baseURI = "https://myplatform.com/teachers/";
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, teacher.name, "/", teacher.photo)) : "";
    }
}
